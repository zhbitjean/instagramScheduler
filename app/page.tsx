...
export default function Home() {
  ...
  const [accountMessage, setAccountMessage] = useState('Instagram will open its official sign-in window.');
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(330px,.65fr)]">
          ...
          <aside className="space-y-5">
            <section className="rounded-3xl border bg-card p-5 shadow-[0_18px_45px_rgb(45_35_80/5%)]">
              <div className="mb-4 flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-gradient-to-br from-[#833ab4] via-[#fd1d1d] to-[#fcb045] text-white"><Camera className="size-4" /></span><div><h2 className="font-semibold">Instagram account</h2><p className="text-xs text-muted-foreground">Log in on Instagram’s secure page</p></div></div>
              {metaSetupOpen ? <div className="space-y-3 rounded-2xl border bg-background p-3"><div><p className="text-sm font-semibold">Developer connection settings</p><p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">This is a one-time Postflow setup—not your Instagram login. Your Instagram password is entered only on Instagram’s next screen.</p></div><div><label className="mb-1 block text-[11px] font-semibold">Instagram app ID</label><Input value={metaAppId} onChange={(event) => setMetaAppId(event.target.value)} inputMode="numeric" placeholder="Numeric app ID from Meta" /></div><div><label className="mb-1 block text-[11px] font-semibold">Instagram app secret</label><Input value={metaAppSecret} onChange={(event) => setMetaAppSecret(event.target.value)} type="password" placeholder="Stored locally" /></div><div><label className="mb-1 block text-[11px] font-semibold">OAuth redirect URL</label><div className="flex gap-2"><Input value={metaRedirectUri} readOnly className="text-[11px]" /><Button variant="outline" size="sm" onClick={() => navigator.clipboard.writeText(metaRedirectUri)}>Copy</Button></div><p className="mt-1 text-[10px] text-muted-foreground">Save this exact URL in Meta’s Instagram business login settings first.</p></div><div className="flex gap-2"><Button variant="outline" className="flex-1" onClick={() => setMetaSetupOpen(false)}>Cancel</Button><Button className="flex-1 bg-[#0064e0] hover:bg-[#0057c2]" onClick={saveMetaSetup}>Save & open Instagram</Button></div><p className="text-[11px] text-muted-foreground">{accountMessage}</p></div> : accountStatus === 'ready' ? <div><div className="flex items-center justify-between rounded-2xl border bg-background p-3"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-full bg-gradient-to-br from-[#7952e8] to-[#ef785b] text-xs font-bold text-white">{instagramUsername.slice(0, 2).toUpperCase()}</span><div><p className="text-sm font-semibold">@{instagramUsername}</p><p className="text-[11px] text-emerald-600">Professional account · selected</p></div></div><Check className="size-4 text-emerald-500" /></div><Button variant="outline" onClick={connectInstagram} className="mt-2 h-9 w-full"><Plus /> Log into another account</Button></div> : <div className="rounded-2xl border bg-background p-3"><div className="mb-3 text-center"><p className="text-sm font-semibold">Log into Instagram</p><p className="mt-1 text-[11px] text-muted-foreground">Use your username, email, or Facebook login on Instagram’s official page.</p></div><Button onClick={connectInstagram} className="h-10 w-full bg-[#4b9bec] text-white hover:bg-[#3789dc]"><Link2 /> Log in to Instagram</Button><p className="mt-2 text-center text-[11px] leading-relaxed text-muted-foreground">{accountMessage}</p></div>}
            </section>
